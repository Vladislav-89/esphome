...
void BL0940::received_package_(const DataPacket *data) const {
  ...
  switch (this->sensor_type_) {
      case CT:
        ...
        total_energy_consumption = cf_cnt;// ((float) to_uint32_t(data->cf_cnt) * 1638.4 * 256 * 1.483524 * (voltage_divider_r2_ + voltage_divider_r1_)) / (3600000 * 4046 * ((CT_load_resistor_ * 1000) / CT_turns_ratio_) * voltage_divider_r1_ * 1000);
      break;
      case SHUNT:
